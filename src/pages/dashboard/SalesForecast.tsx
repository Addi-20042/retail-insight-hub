...
const SalesForecast: React.FC = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useForecast(parseInt(forecastDays));
      ...
      <div className="space-y-8">
        ...
        <div className="chart-container">
          <EmptyState
            ...
            description="Upload sales data first to generate AI-powered demand predictions. The forecasting model needs historical data to make predictions."
          />
        </div>
      </div>
    );
    ...
    <div className="space-y-8">
      ...
      {isError && (
        <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Make sure the Flask backend is running and connected to the database.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="stat-card">
          ...
          <p className="text-2xl font-bold text-foreground mt-1">{isLoading ? '...' : `$${totalPredicted.toLocaleString()}`}</p>
        ...
        <div className="stat-card">
          ...
          <p className="text-2xl font-bold text-foreground mt-1">{isLoading ? '...' : `$${avgDaily.toLocaleString()}`}</p>
        ...
        <div className="stat-card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-foreground mt-1">{isLoading ? '...' : trend ? 'Upward' : 'Downward'}</p>
            </div>
            {!isLoading && (
              trend ? (
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height={400}>
            <AreaChart data={forecastData}>
              ...
              <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '8px' }} formatter={(value: number, name: string) => [`$${value.toLocaleString()}`, name === 'predicted' ? 'Predicted' : name === 'upper' ? 'Upper Bound' : 'Lower Bound']} />
              <Area type="monotone" dataKey="upper" stroke="transparent" fill="hsl(168, 76%, 42%)" fillOpacity={0.1} />
              <Area type="monotone" dataKey="predicted" stroke="hsl(168, 76%, 42%)" strokeWidth={2} fill="url(#colorForecast)" />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
