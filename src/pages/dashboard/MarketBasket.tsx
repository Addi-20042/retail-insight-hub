      ...
      <div className="space-y-8">
        ...
        <div className="chart-container">
          <EmptyState
            ...
            description="Upload sales data with transaction details to discover product associations using Association Rule Mining."
          />
        </div>
      </div>
    );
      ...
      {hasSearched && (
        <div className="space-y-6 animate-slide-in">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="stat-card"><p className="text-sm text-muted-foreground">Rules Found</p><p className="text-2xl font-bold text-foreground">{isLoading ? '...' : searchResults.length}</p></div>
            <div className="stat-card"><p className="text-sm text-muted-foreground">Avg. Confidence</p><p className="text-2xl font-bold text-foreground">{isLoading ? '...' : `${(avgConfidence * 100).toFixed(1)}%`}</p></div>
            <div className="stat-card"><p className="text-sm text-muted-foreground">Avg. Lift</p><p className="text-2xl font-bold text-foreground">{isLoading ? '...' : `${avgLift.toFixed(2)}x`}</p></div>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {searchResults.map((rule, index) => (
                <div key={index} className="bg-card rounded-xl p-5 border border-border/50 hover:shadow-md transition-all hover:-translate-y-0.5">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="px-3 py-2 bg-primary/10 rounded-lg text-primary font-medium">{rule.productA}</div>
                    <ArrowRight className="w-5 h-5 text-muted-foreground" />
                    <div className="px-3 py-2 bg-chart-secondary/10 rounded-lg text-chart-secondary font-medium">{rule.productB}</div>
                  </div>
            ...
            <div className="chart-container text-center py-12"><p className="text-muted-foreground">No associations found for "{searchQuery}"</p></div>
          )}
        </div>
      )}
    </div>
